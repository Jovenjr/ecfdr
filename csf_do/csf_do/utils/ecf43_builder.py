"""
Generador 1:1 del XML para e-CF 43 (Gastos Menores) basado en el XSD "e-CF 43 v.1.0.xsd".

Construye la estructura mínima requerida por el esquema:
- ECF/Encabezado/Version, IdDoc(TipoeCF, eNCF, FechaVencimientoSecuencia, [TipoPago], [TotalPaginas])
- ECF/Encabezado/Emisor(...campos requeridos...)
- ECF/Encabezado/Totales(MontoTotal requerido; otros opcionales)
- ECF/DetallesItems/Item(...campos requeridos...)
- ECF/FechaHoraFirma (requerido)
- ECF/(xs:any) → insertamos un nodo "Signature" placeholder (se reemplaza al firmar realmente)

Validación: usar csf_do.csf_do.utils.xsd_validator.validate_xml con "e-CF 43 v.1.0.xsd".

Notas importantes DGII:
- No incluir tags vacíos: sólo crear nodos si hay valor (excepto los requeridos por minOccurs=1).
- Escapar caracteres especiales en campos alfanuméricos.
- Redondeos/tolerancias y cálculos de impuestos se integrarán en una etapa posterior.
"""
...
# Catálogos disponibles (archivo JSON dentro de csf_do.csf_do.data)
CATALOG_FILES = {
...
def _load_catalog_values(catalog_json_file: str) -> List[str]:
    ...
    try:
        ...
        # Catálogo no disponible aún (no bloquear construcción, pero sí advertir si se desea)
        return []


def _validate_catalog_value(field: str, value: Optional[str], *, catalog_type: str) -> None:
    """Valida que 'value' pertenezca al catálogo 'catalog_type' si el catálogo está disponible.
    Si el catálogo no existe aún, no bloquea.
    """
...
def _to_dmy(date_str: str) -> str:
    """Convierte 'YYYY-MM-DD' a 'DD-MM-YYYY' si aplica. Si no, retorna tal cual."""
    s = str(date_str)
    try:
...
def _to_dmy_hms(datetime_str: str) -> str:
    """Convierte 'YYYY-MM-DDTHH:MM:SS' a 'DD-MM-YYYY HH:MM:SS' si aplica. Si no, retorna tal cual."""
    s = str(datetime_str)
    # Intento ISO con 'T'
    try:
...
def build_ecf43_xml(data: Dict) -> str:
    """Construye el XML de e-CF 43 cumpliendo el XSD base.

    data: dict con claves esperadas, por ejemplo:
      encabezado: {
        Version, IdDoc: {TipoeCF, eNCF, FechaVencimientoSecuencia, TipoPago?, TotalPaginas?},
        Emisor: {RNCEmisor, RazonSocialEmisor, DireccionEmisor, FechaEmision, ...opcionales},
        Totales: {MontoTotal, MontoExento?, ...},
        OtraMoneda?: {TipoMoneda?, TipoCambio?, MontoExentoOtraMoneda?, MontoTotalOtraMoneda?}
      }
      detalles: { items: [ {NumeroLinea, IndicadorFacturacion, NombreItem, IndicadorBienoServicio, CantidadItem, PrecioUnitarioItem, MontoItem, ...opcionales} ] }
      subtotales?: {...}
      paginacion?: {...}
      informacion_referencia?: {...}
      fecha_hora_firma: "YYYY-MM-DDThh:mm:ss"
    """
    # Validación previa contra spec (si existe)
    try:
        errs = validate_against_spec(data, spec_name="ecf43.json")
        if errs:
    ...
    for key in REQUIRED_IDDOC_FIELDS:
        ...
        # Normalizar fechas según XSD
        if key == "FechaVencimientoSecuencia":
            text_val = _to_dmy(str(val))
        else:
            text_val = str(val)
    ...
    # Opcionales
    _add_if_present(iddoc, "TipoPago", iddoc_in.get("TipoPago"))
    _add_if_present(iddoc, "TotalPaginas", iddoc_in.get("TotalPaginas"))

    # Emisor (obligatorio con campos requeridos)
    ...
    # Validar catálogo Provincia/Municipio si se proveen
    ...
    # Totales (obligatorio; MontoTotal requerido)
    ...
    _add_if_present(totales, "MontoPeriodo", totales_in.get("MontoPeriodo"))
    _add_if_present(totales, "SaldoAnterior", totales_in.get("SaldoAnterior"))
    _add_if_present(totales, "MontoAvancePago", totales_in.get("MontoAvancePago"))
    _add_if_present(totales, "ValorPagar", totales_in.get("ValorPagar"))
    ...
    # DetallesItems (obligatorio; al menos 1 Item)
    ...
    for it_in in items:
        ...
        if it_in.get("TablaCodigosItem"):
            ...
            # espera lista de dicts con {TipoCodigo, CodigoItem}
    ...
    # Subtotales (opcional) — no implementado aún; se puede extender aquí

    # Paginacion (opcional) — no implementado aún; se puede extender aquí

    # InformacionReferencia (opcional) — no implementado aún; se puede extender aquí
    ...
    # xs:any (obligatorio) → colocamos un nodo Signature placeholder con texto no vacío
    # Nota: si queda vacío, remove_empty_elements lo eliminaría y el XSD fallaría por faltar xs:any
    ET.SubElement(root, "Signature").text = "-"  # se reemplaza al firmar realmente

    # Limpiar tags vacíos (manteniendo los requeridos)
    remove_empty_elements(root)

    return ET.tostring(root, encoding="utf-8").decode("utf-8")


def build_and_validate_ecf43(data: Dict) -> Tuple[str, bool, List[str]]:
    """Construye el XML para e-CF 43 y lo valida contra el XSD.
    Retorna: (xml, ok, errores)
    """
    xml = build_ecf43_xml(data)
    ok, errors = validate_xml(xml, xsd_name="e-CF 43 v.1.0.xsd")
