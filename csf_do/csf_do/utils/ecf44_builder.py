...
def build_ecf44_xml(data: Dict) -> str:
    try:
        errs = validate_against_spec(data, spec_name="ecf44.json")
        if errs:
    ...
    for it_in in items:
        ...
        # En 44, el XSD del Item base no contempla UnidadMedida; no lo incluimos
    ...
    # Agregación de impuestos adicionales a nivel de Totales (44 exige Tasa en cada ImpuestoAdicional)
    if items_taxes_acc:
        agg = aggregate_additional_taxes(items_taxes_acc)
        agg_con_tasa = {k: v for k, v in agg.items() if v.get("tasa")}
        if agg_con_tasa:
            # Primero MontoImpuestoAdicional, luego ImpuestosAdicionales según orden XSD
    ...
    # Mover MontoTotal al final del bloque Totales según XSD
...
def build_and_validate_ecf44(data: Dict) -> Tuple[str, bool, List[str]]:
    xml = build_ecf44_xml(data)
    ok, errors = validate_xml(xml, xsd_name="e-CF 44 v.1.0.xsd")
